...
class PipelineEvaluator:
    ...
    def is_valid_desmos_latex(latex_str: str) -> Tuple[bool, str]:
        ...
        # Delegate schema validation (LHS, non-empty pairs/brackets/fractions) to DesmosExpression validator
        try:
            DesmosExpression.validate_complete_latex(latex_str)
        except ValueError as ve:
            return False, str(ve)

        # Verify no literal English prose inside LaTeX expression
        if re.search(r'\b(?:the|graph|of|a\s+cubic|a\s+quadratic|a\s+line|a\s+function|polynomial|shaded|area|under|curve|point|approaching)\b', latex_str, re.IGNORECASE):
            return False, "Contains literal English prose text instead of pure LaTeX math."
